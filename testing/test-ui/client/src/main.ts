...
const loadPerfectStack = async (stackConfig: NgxPerfectStackConfig) => {
  try {
    const metaRoleResponse = await fetch(`${stackConfig.apiUrl}/meta/role`);
    const metaRoleJson = await metaRoleResponse.json();
